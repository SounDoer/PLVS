...
if (!m) {
  console.error("Cargo.toml: package version not found in [workspace.package] block");
  process.exit(1);
}
...
if (pkgVersion !== cargoVersion || pkgVersion !== tauriVersion || pkgVersion !== lockVersion) {
  console.error(
    "Version mismatch — package.json, src-tauri/Cargo.toml [workspace.package], src-tauri/tauri.conf.json, package-lock.json must match:"
  );
