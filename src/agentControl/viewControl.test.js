...
describe("View Control", () => {
  it("describes public values, schema, runtime ownership, and availability", () => {
    expect(buildViewDescription(current, { platform: "windows", docked: true })).toMatchObject({
      ...
      schema: {
        surfaceOpacity: { type: "integer", minimum: 0, maximum: 100, current: 100 },
      },
    });
  ...
  it("strictly validates and merge-patches Focus View atomically", () => {
    const planned = planViewUpdate(
      current,
      { pinned: true, focusView: { compactPanels: true }, surfaceOpacity: 85 },
      { platform: "windows" }
    );
    expect(planned).toMatchObject({
      ...
      changed: ["view.pinned", "view.focusView.compactPanels", "view.surfaceOpacity"],
      effects: ["alwaysOnTop", "compactPanels", "surfaceOpacity"],
      issues: [],
    });
    expect(
      planViewUpdate(
        current,
        { focusView: { compactPanels: "yes" }, surfaceOpacity: 84.5 },
        { platform: "windows" }
      )
    ).toMatchObject({
      view: current,
      ...
      issues: [
        ...
        { code: "invalidRange", path: "$.surfaceOpacity" },
      ],
    });
  });
  ...
  it("warns for changed window presentation fields while Dock owns the form", () => {
    const planned = planViewUpdate(
      current,
      { pinned: true, focusView: { borderless: true }, surfaceOpacity: 75 },
