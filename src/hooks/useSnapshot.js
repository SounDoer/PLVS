...
export function useSnapshot({
  ...
  const fallbackSnapIdx =
    selectedHistSteps >= 0 ? Math.max(0, snapSpecDataList.length - 1 - selectedHistSteps) : -1;
  const snapIdx =
  ...
  const displaySpectrumPath = (() => {
    ...
    // No visual snapshot (only the first ~40 ms before the visual ring fills): live path.
    return spectrumPath;
  })();
