...
| Spectrogram | 继承 Spectrum                          | 2D 已优化（−87%/−95%，92.0% 脏帧走滚动条带）；3D Lines 已优化（脊线走 hairline 快路径，GPU 14–17% → 4.6%，GPU 进程 CPU 330 → 5.5 ms/s）；3D Surface 已迁到 WebGL（主线程 7.10 → 3.09 ms/次）；绘制窗口已从 10 Hz master 停顿改为跟随 25 Hz visual 时间戳；轮廓沸腾接受现状 | 继承 Spectrum                                           | 继承 Spectrum                                |
| Stereo Map  | 已测，约 50.6 µs/批；零分配，不改      | 已测，派生 <0.11 ms；canvas 调度合理                                                                                                                                                                                                    | 已优化，1.29 → 0.647 GiB/key（约 −50%）；4-bit 视觉已验 | 已落地（栅格 −23%，行再 −70%）；真实窗口已验 |
| Waveform    | 边界、正确性与成本均已测；48 kHz 立体声约 0.025% 单核 | 已测并优化三处（谱线 seek、默认不计算、颜色循环），已在真实窗口验证                                                                                                                                                                     | 已测，占历史约 1%，拒绝                                 | 已测，11.29 KiB/s，拒绝                      |
