...
**后续已补可观测性：** `get_ui_frame_diagnostics` 返回本 session 的发送数、背压丢弃数、当前/最高
in-flight 与 120 帧上限。计数发生在真实 send/drop 分支；不再拿连续 `seq` 误判为上游没有丢帧。
2026-09-01 的 Heavy 整机 30 分钟基线中，该计数为 0，积压峰值 39/120；故意阻塞 WebView
4 秒的对照验收则精确到达 120，记录 85 个丢帧，恢复 2 秒后当前积压回到 1。
...
3. **接 Spectrum 一条路**，其余数组仍走 JSON（信封天然支持混合）。
   **已完成并在真实窗口验证**（§9）：`src-tauri/src/ipc/frame_encode.rs` 造信封，
...
4. **接 Stereo Map**。**已完成并在真实窗口验证**（§9）：`pl` / `pr` / `c` 三行走 `f32` section。
...
7. **dB 行窄到 f32（P-7）**。`SpectrumFrameResult` / `SpectrumVisualEntry` 的行类型从
   ...
   所以 section 是零拷贝的，JSON 那条路也一并变小。**已完成并在真实窗口验证**（§9）。
...
|                    |                                                          |
| ------------------ | -------------------------------------------------------- |
| 设备               | `CABLE Output (VB-Audio Virtual Cable)`，48000 Hz，2 ch  |
| Dropped            | **0**                                                    |
| `integratedLufs`   | file −22.030555 / live −22.033827（差 0.0033，容差 0.5） |
| `samplePeakMaxLDb` | **逐位一致**                                             |
| `samplePeakMaxRDb` | **逐位一致**                                             |

跑了两次都过：一次在连接状态，一次在会话分离状态。

**但它覆盖不到本轮的改动**，这点要说清楚：`smoke:capture` 走 `plvs-cli capture` →
`capture_device_to_summary`，用的是它自己的 `SummaryMeter` 消费者，**完全不碰**
`frame_subscribers`、`encode_audio_frame` 或 `Raw` channel。它与实时路径共享设备层和 DSP，
**不共享发送点**。本轮改动的实时路径由 §9.7 覆盖，不是由它。
它仍然必须过，因为这轮动了 `src-tauri/src/audio`，而它是采集层的发布门。

### 10.2 `soak:capture`（4 小时）

|                  |                            |
| ---------------- | -------------------------- |
| 样本             | 1441（post-warmup 1434）   |
| 静音样本         | **0**                      |
| `droppedChunks`  | **0**                      |
| `integratedLufs` | −22.035613 → −22.037364    |
| **漂移**         | **0.0036 dB**（上限 0.01） |
| RSS              | **8.3 → 8.5 MB**           |

**判定：通过。** 历史 5 次留存记录的漂移是 0.0028–0.0034 dB，这次 0.0036 只高出 0.0002
（比最高值高 6%）——落在噪声里，而且这个统计量是 `max − min`，本身混着积分响度的收敛尾巴。
不作为信号读。

RSS +0.2 MB / 4h，与历史同档（8.5→7.9、9.1→9.2）。脚本自己注明 `SummaryMeter` 按设计就会累积
约 3.5 MB，这次远低于。**本轮把每帧的 payload clone 换成了 bytes clone，没有留下泄漏。**

### 10.3 分离会话里跑采集：两个反直觉的事实

**(a) 断开 RDP 不会让 VB-Cable 出现——除非改音频重定向设置。**
...
用引擎自己的枚举每 15 秒查一次、连查 **2 小时**，始终是 0 个。不是"还没稳定"。
（**不要用 PowerShell 代替这个探测**——它能解析出端点而 WASAPI 看不见，两者会给出相反的答案。）

**把 RDP 客户端的音频改成在远程计算机上播放之后，5 个设备全部出现，并且在断开和重连时都保住。**
这比 `tscon <id> /dest:console`（需提权、切断远程访问、系统级）便宜得多，是首选做法。

**(b) 断开前启动的播放器活不过会话切换——而它不会报错。**

soak 在会话分离的瞬间变成了采集静音：设备在、采集成功、`droppedChunks: 0`，
但每个样本的 `integratedLufs` 都是 `null`。VLC 进程仍然活着，它的音频却不再进入 VB-Cable。

**这个失败模式很坏**：不抛错、不掉帧、进程俱在，只是安静地采空气。只盯"进程在吗/设备在吗/
有掉帧吗"的看门狗会一路绿到最后，然后拿一份没有信号的记录去判定"没有漂移"。

**断开之后重新启动播放器就完全正常**（−22.036 / −19.999）。所以规则是：
**分离会话里跑 rig，播放器必须在断开之后启动**，并且看门狗要显式检查 `integratedLufs` 是否为
`null`，而不是只看进程和掉帧。
