...
当前纯 JS 派生不超 0.11 ms；D3 的四小时单 key、单 mode 已从 **1.29 GiB 经 0.97 / 0.81 GiB
降到 0.647 GiB（累计约 −50%）**，4-bit alpha 阶梯已在真实窗口验收；D4 先通过共享帧级频率栅格
...
**真实窗口已验证**：现役 4-bit（最大 alpha 误差 0.038）未观察到可辨阶梯，采用 4-bit。
...
**真实窗口已验证**：统一协议的 WebView2 解码从 0.689 降到 0.073 ms/帧，主线程约省
38.5 ms/s；完整结果见 `protocol.md` §9.3。
