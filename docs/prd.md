...
- **频谱**：采用 **FFT 型 RTA 的常见工程实践**（带内能量按 **Hz 连续边界** 与各 FFT bin 的**分数重叠**聚合，而非整档扣整数 bin）；纵轴为 **dBFS 域**带内谱功率（与 Peak 的采样峰值 dBFS **同参考域、不同定义**）；**不声称** IEC 61260 滤波器组计量路径；与响度 **不可横向等同**。第一版对外承诺 **Spectrum 为固定口径的参考视图（B）**，在下半记录具体口径与可调项边界；**单位与参考的完整表述**见 **`docs/architecture.md`** §6 小节 **「纵轴单位与参考（dBFS）」**。  
- **多声道（目标演进）**  
