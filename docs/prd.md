...
提供一个 **独立桌面应用**：用户选择 **输入源（含 Automatic）** 后开始监测；在同一界面中同时查看 **多块表头**（Level Meter / Loudness / Stats / Spectrum / Spectrogram / Vectorscope / Waveform）。  
...
- **表头**：**Level Meter / Loudness / Stats / Spectrum / Spectrogram / Vectorscope / Waveform** 同屏；默认 **英文 UI**；**主题**默认 **跟随系统**，保留 **Light/Dark**，并已支持 **自定义主题与主题编辑器**。  
...
1. ~~**离线音频文件分析**~~ —— **已撤销（2026-06）**：File 模式已交付，支持本地音频文件离线分析；仍为 read-only、不处理音频。实时监测仍是核心。详见附录 A.1。  
...
- **多声道**：Level Meter 逐通道显示；最多 8 声道时按数量自动识别，手动布局覆盖 mono 至 9.1.6（BS.1770-5 权重，WAVE / ffmpeg 顺序）；未知布局降级 Ch1/Ch2 并显示 `Ch 1–2` 标记与 footer 提示；True Peak Max 覆盖全部声道；Spectrum 与 Vectorscope 支持声道/声道对选择。  
...
- **无障碍增强**：非短期主线（见 5.11）。  
- **一键诊断导出**：非承诺项（见 5.9）。  
- **手动布局选择已交付**：Settings 的 **Channels · Layout**、Agent Control 的 `settings.channelLabels.layout` 与内部 CLI 的 `--layout` 均可选择 mono 至 9.1.6；逐声道角色编辑仍用于校正来源声明或声道顺序。  
