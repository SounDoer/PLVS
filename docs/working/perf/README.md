...
| Spectrogram | 继承 Spectrum                                | 2D 已优化（−87%/−95%，92.0% 脏帧走滚动条带）；3D Lines 已优化（脊线走 hairline 快路径，GPU 14–17% → 3.0%，GPU 进程 CPU 330 → 17 ms/s）；3D Surface 383 ms/s 在渲染进程，待决策 | 继承 Spectrum                         | 继承 Spectrum                         |
