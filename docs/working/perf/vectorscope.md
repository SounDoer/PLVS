...
**状态：** 四维已测，尚未改生产实现。D1 有一个输出等价的字符串构建候选；D2 找到两类
确定的冗余（Polar 预取了不用的 Persistence 窗口、canvas 每次 render 同步读布局）；D3 已经
...
### 2.4 Polar 每个 visual tick 还解码了一份完全不用的 1 秒窗口

`needsHistorySlab` 在 Polar 模式为真；随后 `persistenceRows` 无条件调用
...
只把 Persistence 选择挂到 Lissajous 上，就能去掉 Polar 历史准备约 **70% 的 CPU**和
**520 KiB/s** 分配（按 25 Hz）。这是输出完全等价的确定冗余，**值得与 canvas 调度分开提交**。

另有两条次级项：

- Polar Level 最后只读 180 ms（5 行），但父层先按 Polar Sample 的 400 ms 解码 11 行，再 filter；
  可以直接按 180 ms 选窗，省剩余解码约 55%。
- `DockVectorscope` 直接在 render 中调用 `selectPolarWindow`，没有按 slab version memo；按 60 Hz
  父帧折算约 6 ms CPU/s 和 0.5 MiB/s 分配。应与主 panel 的选窗修复一起覆盖。
...
| D2-4 | Polar 每 tick 解码不用的 1 秒 Persistence 窗口   | 0.239 ms + 20.8 KiB/次                     | **值得改**             |
| D2-5 | Polar Level 多解码 6/11 行；Dock 每 render 选窗  | benchmark + 读码                           | 与 D2-4 一起改         |
...
1. **前端选窗去冗余**：Polar 不再构造 Persistence 窗口，Level 直接选 180 ms，Dock 按 version memo。
2. **canvas 尺寸/绘制调度**：ResizeObserver 驱动 backing size；Persistence 和 Polar 不再每 render
   同步读布局。
3. **Rust path 直接写 String**：输出逐字节对拍后替换 `Vec<String> + join`。
4. **统一二进制协议轮**：与 Spectrum 第 3 层一起讨论；届时清掉三个未读 visual metrics。
