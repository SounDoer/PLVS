...
| P-3     | Rust 侧省下的 `serde_json` 格式化时间                 | Criterion 整帧序列化对拍。**未做**                                 |
...
在一台断开的 RDP 会话里跑的：会话保留、应用照常渲染，但**音频端点全部消失**——`plvs-cli devices`
返回 0 个设备，VB-Cable 驱动状态是 OK 却不对 WASAPI 可见。所以**实时采集路径没能验证**，
`smoke:capture` 与 `soak:capture` 都跑不了（见 §10）。文件分析这条路不需要任何音频设备，
下面的全部结论出自它。
...
- P-3（Rust 侧省下的序列化时间）—— 未做，需要 Criterion。

回到能看见 VB-Cable 的会话后，这三项要补。
