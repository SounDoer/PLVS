...
| **STFT 栅格与帧平均** | 环缓冲满后，按 **固定 hop = N/4**（与 **N** 成比例，当前 `N=4096` → hop=1024 采样）的**已摄入帧计数**触发 FFT，与单次设备回调块长解耦；对最近 **4** 次 STFT 的**各档线性功率**做算术平均后再 `10·log10`（**非相干**时间平均），再接 **Z/A/C**、邻频核与 attack/release。首段未满 4 帧时用已有帧数平均。 |
...
| 2026-05 | — | §6 Spectrum：**STFT** 固定 **hop=N/4** + **4 帧**带内线性功率非相干平均（`spectrum.rs`） |
